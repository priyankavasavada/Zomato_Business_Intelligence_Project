...
-- =====================================================================
-- Check for duplicate Primary Keys across core entities
SELECT 'cities' AS table_name, CityID AS pk_id, COUNT(*) FROM cities GROUP BY CityID HAVING COUNT(*) > 1
UNION ALL
SELECT 'customers', CustomerID, COUNT(*) FROM customers GROUP BY CustomerID HAVING COUNT(*) > 1
UNION ALL
SELECT 'restaurants', RestaurantID, COUNT(*) FROM restaurants GROUP BY RestaurantID HAVING COUNT(*) > 1
UNION ALL
SELECT 'orders', OrderID, COUNT(*) FROM orders GROUP BY OrderID HAVING COUNT(*) > 1;

-- Check for duplicate alternate unique keys
SELECT CouponCode, COUNT(*) FROM promotions GROUP BY CouponCode HAVING COUNT(*) > 1;
SELECT City, COUNT(*) FROM cities GROUP BY City HAVING COUNT(*) > 1;

--Identify orphaned child records that reference parent IDs that do not exist
--Orphaned Customers / Restaurants / Delivery Partners in Orders
SELECT 'orders -> customers' AS fk_link, COUNT(*) AS orphaned_count 
FROM orders o LEFT JOIN customers c ON o.CustomerID = c.CustomerID WHERE c.CustomerID IS NULL
UNION ALL
SELECT 'orders -> restaurants', COUNT(*) 
FROM orders o LEFT JOIN restaurants r ON o.RestaurantID = r.RestaurantID WHERE r.RestaurantID IS NULL
UNION ALL
SELECT 'orders -> delivery_partners', COUNT(*) 
FROM orders o LEFT JOIN delivery_partners dp ON o.DeliveryPartnerID = dp.DeliveryPartnerID WHERE dp.DeliveryPartnerID IS NULL;

-- Orphaned Cities in Entities
SELECT 'customers -> cities' AS fk_link, COUNT(*) 
FROM customers c LEFT JOIN cities ct ON c.City = ct.City WHERE ct.City IS NULL
UNION ALL
SELECT 'restaurants -> cities', COUNT(*) 
FROM restaurants r LEFT JOIN cities ct ON r.City = ct.City WHERE ct.City IS NULL
UNION ALL
SELECT 'delivery_partners -> cities', COUNT(*) 
FROM delivery_partners dp LEFT JOIN cities ct ON dp.City = ct.City WHERE ct.City IS NULL;

-- Orphaned Order Line Items & Financial Records
SELECT 'order_items -> orders' AS fk_link, COUNT(*) 
FROM order_items oi LEFT JOIN orders o ON oi.OrderID = o.OrderID WHERE o.OrderID IS NULL
UNION ALL
SELECT 'payments -> orders', COUNT(*) 
FROM payments p LEFT JOIN orders o ON p.OrderID = o.OrderID WHERE o.OrderID IS NULL
UNION ALL
SELECT 'customer_feedback -> orders', COUNT(*) 
FROM customer_feedback f LEFT JOIN orders o ON f.OrderID = o.OrderID WHERE o.OrderID IS NULL;

--Identify any records with NULL values in transactional fields
SELECT 
    COUNT(*) FILTER (WHERE OrderDate IS NULL) AS null_order_dates,
    COUNT(*) FILTER (WHERE OrderTime IS NULL) AS null_order_times,
    COUNT(*) FILTER (WHERE OrderStatus IS NULL) AS null_order_statuses,
    COUNT(*) FILTER (WHERE FinalAmount IS NULL) AS null_final_amounts
FROM orders;